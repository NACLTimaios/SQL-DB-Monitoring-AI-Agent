...
export default function DatabaseSummaryPanel({ summary }: any) {
  ...
  return (
    <div className="h-full bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 rounded-xl p-4 flex flex-col">
      <h3 className="text-sm font-semibold text-slate-300 mb-3">Database Metrics</h3>

      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="bg-slate-900/50 p-2 rounded">
          <p className="text-xs text-slate-400">Connections</p>
          <p className={`text-sm font-semibold ${connColor}`}>
            {summary.connections}/{summary.connections_max}
          </p>
        </div>
        <div className="bg-slate-900/50 p-2 rounded">
          <p className="text-xs text-slate-400">Disk Used</p>
          <p className="text-sm font-semibold text-slate-200">{summary.disk_size_gb?.toFixed(2)} GB</p>
        </div>
      </div>
